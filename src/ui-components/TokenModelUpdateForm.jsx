...
export default function TokenModelUpdateForm(props) {
  const {
    id: idProp,
    tokenModel: tokenModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tokenModelRecord
      ? { ...initialValues, ...tokenModelRecord }
      : initialValues;
    setOrg_id(cleanValues.org_id);
    setAccess_token(cleanValues.access_token);
    setExpires_in(cleanValues.expires_in);
    setRefresh_token(cleanValues.refresh_token);
    setRefresh_token_expires_in(cleanValues.refresh_token_expires_in);
    setToken_type(cleanValues.token_type);
    setErrors({});
  };
  const [tokenModelRecord, setTokenModelRecord] =
    React.useState(tokenModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TokenModel, idProp)
        : tokenModelModelProp;
      setTokenModelRecord(record);
    };
    queryData();
  }, [idProp, tokenModelModelProp]);
  React.useEffect(resetStateValues, [tokenModelRecord]);
  const validations = {
    org_id: [],
    access_token: [],
    expires_in: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TokenModel.copyOf(tokenModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TokenModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Org id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tokenModelModelProp)}
