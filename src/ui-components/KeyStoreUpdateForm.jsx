...
export default function KeyStoreUpdateForm(props) {
  const {
    id: idProp,
    keyStore: keyStoreModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = keyStoreRecord
      ? { ...initialValues, ...keyStoreRecord }
      : initialValues;
  ...
  const [keyStoreRecord, setKeyStoreRecord] = React.useState(keyStoreModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(KeyStore, idProp)
        : keyStoreModelProp;
      setKeyStoreRecord(record);
    };
    queryData();
  }, [idProp, keyStoreModelProp]);
  React.useEffect(resetStateValues, [keyStoreRecord]);
  const validations = {
    org_id: [],
    access_token: [],
    expires_in: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            KeyStore.copyOf(keyStoreRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "KeyStoreUpdateForm")}
      {...rest}
    >
      <TextField
        label="Org id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || keyStoreModelProp)}
