...
export default function TokenStoreUpdateForm(props) {
  const {
    id: idProp,
    tokenStore: tokenStoreModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tokenStoreRecord
      ? { ...initialValues, ...tokenStoreRecord }
      : initialValues;
  ...
  const [tokenStoreRecord, setTokenStoreRecord] =
    React.useState(tokenStoreModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TokenStore, idProp)
        : tokenStoreModelProp;
      setTokenStoreRecord(record);
    };
    queryData();
  }, [idProp, tokenStoreModelProp]);
  React.useEffect(resetStateValues, [tokenStoreRecord]);
  const validations = {
    org_id: [],
    access_token: [],
    expires_in: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TokenStore.copyOf(tokenStoreRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TokenStoreUpdateForm")}
      {...rest}
    >
      <TextField
        label="Org id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tokenStoreModelProp)}
