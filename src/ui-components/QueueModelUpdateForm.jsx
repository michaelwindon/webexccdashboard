...
import { getQueueModel } from "../graphql/queries";
import { updateQueueModel } from "../graphql/mutations";
...
export default function QueueModelUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = queueModelRecord
      ? { ...initialValues, ...queueModelRecord }
      : initialValues;
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getQueueModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getQueueModel
        : queueModelModelProp;
  ...
  React.useEffect(resetStateValues, [queueModelRecord]);
  const validations = {
    name: [],
    group: [],
  };
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateQueueModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: queueModelRecord.id,
                ...modelFields,
              },
            },
          });
